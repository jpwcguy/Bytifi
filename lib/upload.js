...
async function uploadMultipartStreaming(filePath, context, {
  ...
  // AbortSignal.any rejects undefined entries — only include a caller signal when present.
  ...
  try {
    ...
    if (!sessionAborted) {
      sessionAborted = true
      // Never reuse the caller's aborted signal — cleanup must still reach the API.
      await abortUploadSession(baseUrl, sessionToken, { apiKey, signal: undefined })
    }
